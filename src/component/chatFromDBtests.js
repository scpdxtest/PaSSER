...
const ChatFromDBtests = () => {
    ...
    const embeddings_open = new OllamaEmbeddings({
        model: selectedModel, //"mistral", // default value
        // baseUrl: "http://195.230.127.227:11434", // default value  
        // baseUrl: "http://127.0.0.1:11434", // default value  
        baseUrl: selectedOllama //"http://93.152.201.115:11434", // default value  
      });

    const mdl = new Ollama({
        // baseUrl: "http://127.0.0.1:11434",
        // baseUrl: "http://195.230.127.227:11434", // default value  
        baseUrl: selectedOllama, //"http://93.152.201.115:11434",
    ...
    const listCollections = async (ch) => {
        ...
        // console.log("collectionsWithIdAndName", collectionsWithIdAndName);
    ...
    const onRowDoubleClick = async (e) => {
        ...
        console.log("collection", e.data.name);
    ...
    const handleSubmit = async () => {
        ...
        const llm = new Ollama({baseUrl: selectedOllama, model: selectedModel});
        const OPENAI_API_KEY = "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";
        ...
        const chain_test = await loadEvaluator("embedding_distance");

        const res_rest = await chain_test.evaluateStrings({
            prediction: expectedAnswer,
            reference: res.text,
        });
        console.log("Cosine distance", { res_rest});
        ...
        setMessages(prevMessages => {
            ...
            // responseObject.response = ''; // Clear the response
            return newMessages;
        });
    ...
    return (
        ...
        {dialogVisible &&
            <div style={{ width: '90%', marginLeft: '20px'}}>
                <InputText style={{ width: '90%' }} value={expectedAnswer} onChange={(e) => setExpectedAnswer(e.target.value)} />
...
export default ChatFromDBtests;